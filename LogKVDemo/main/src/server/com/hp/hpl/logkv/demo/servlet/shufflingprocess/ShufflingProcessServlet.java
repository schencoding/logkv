 ...
 * @author Edmond
 */
public class ShufflingProcessServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			Object requestObj = reader.readObject();
			...
			if (requestObj instanceof ShufflingProcessWrapRequest) {
				ShufflingProcessWrapRequest wrapRequestObj = (ShufflingProcessWrapRequest) requestObj;
				requestClientID = wrapRequestObj.requestClientId;
				jmsSender.send(wrapRequestObj.requestClientId,  wrapRequestObj.shufflingStatusRequest);
			} else {
				requestClientID = UUID.randomUUID().toString();
				jmsSender.send(requestClientID,  (Serializable) requestObj);
			}
